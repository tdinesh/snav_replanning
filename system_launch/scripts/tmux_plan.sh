...
tmux split-window -t $SESSION_NAME
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; roslaunch snavquad_interface pc_filter.launch mav_name:=${MAV_NAME}" Enter
tmux split-window -t $SESSION_NAME
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 4; roslaunch snavquad_interface octomap_mapping.launch mav_name:=${MAV_NAME}"
...
tmux new-window -t $SESSION_NAME -n "Fp"
#tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; roslaunch man_localize pc_filter.launch mav_name:=${MAV_NAME}" Enter
#tmux split-window -t $SESSION_NAME
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; roslaunch man_localize fp_pub.launch" Enter
tmux split-window -t $SESSION_NAME
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; roslaunch man_localize sbim_levine.launch" Enter
tmux split-window -t $SESSION_NAME
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; roslaunch man_localize fp_localize.launch" Enter
tmux split-window -t $SESSION_NAME
tmux send-keys -t $SESSION_NAME "$SETUP_ROS_STRING; sleep 5; roslaunch man_localize wp_transform.launch mav_name:=${MAV_NAME}" Enter
